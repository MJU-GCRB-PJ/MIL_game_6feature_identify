...
ENSEMBLE_PY = SCRIPT_DIR / "08_ensemble.py"
...
def main():
    print(f"=== Ablation Study: TARGET FOLDER `{TARGET_FOLDER}`, ENSEMBLE ROW No `{TARGET_ENSEMBLE_ROW}` ===")
    ...
    row_mask = df_results['No'] == TARGET_ENSEMBLE_ROW
    if not row_mask.any():
        raise ValueError(f"All_Results has no row where No == {TARGET_ENSEMBLE_ROW}.")
    ...
    ensemble_mod.TARGET_FOLDER = TARGET_FOLDER
    ensemble_mod.OUTPUT_BASE = output_dir
    ensemble_mod.DATA_CSV = output_dir / "splits" / "feat_data-ration_list.csv"
    ...
    ablation_out_dir = output_dir / "ablation_result"
    ablation_out_dir.mkdir(parents=True, exist_ok=True)
    out_file = ablation_out_dir / f"ablation_row_{TARGET_ENSEMBLE_ROW}_results.xlsx"
    ...
    plt.title(f"Ablation Results for Ensemble Row {TARGET_ENSEMBLE_ROW}\n({combo_str})", fontsize=18, fontweight='bold')
    

    plt.subplots_adjust(left=0.15, right=0.85, top=0.9, bottom=0.1)
    
    # Save output.
    plot_file = ablation_out_dir / f"ablation_row_{TARGET_ENSEMBLE_ROW}_trend.png"
